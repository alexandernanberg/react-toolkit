...
class FriendlyOutputWebpackPlugin {
  constructor(options = {}) {
  ...
  apply(compiler) {
    ...
    const plugin = { name: 'FriendlyOutputWebpackPlugin' }
  ...
  displayErrors(errors, severity) {
    ...
    const subtitle =
      severity === 'error'
        ? `Failed to compile with ${nbErrors} error${nbErrors ? 's' : ''}`
        : `Compiled with ${nbErrors} warning${nbErrors ? 's' : ''}`
    output.title(severity, severity, subtitle)
...
module.exports = FriendlyOutputWebpackPlugin
